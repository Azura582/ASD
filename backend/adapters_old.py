...
_image_model = None
_image_framework = None  # 'keras' or 'torch'
_class_names = []
...
def load_adapters():
    ...
    # 加载图片模型（支持 Keras 和 PyTorch）
    try:
        if not IMAGE_MODEL_PATH.exists():
            _image_model = None
            _image_framework = None
            print(f"[adapters] 警告: 图片模型未找到于 {IMAGE_MODEL_PATH}")
        else:
            suffix = IMAGE_MODEL_PATH.suffix.lower()
            
            # 优先尝试 PyTorch (.pt, .pth, .jit)
            if suffix in ('.pt', '.pth', '.jit') and _torch is not None:
                try:
                    # 尝试 TorchScript 加载
                    try:
                        _image_model = _torch.jit.load(str(IMAGE_MODEL_PATH), map_location='cpu')
                    except Exception:
                        # 回退到普通 torch.load (设置 weights_only=False 以支持自定义类)
                        _image_model = _torch.load(str(IMAGE_MODEL_PATH), map_location='cpu', weights_only=False)
                    
                    _image_model.eval()
                    _image_framework = 'torch'
                    print(f"[adapters] ✓ 图片模型已用 PyTorch 加载 ({IMAGE_MODEL_PATH.name})")
                except Exception as e:
                    _image_model = None
                    _image_framework = None
                    print(f"[adapters] 错误: 用 PyTorch 加载图片模型失败: {e}")
                    traceback.print_exc()
            
            # Keras 格式 (.keras, .h5, .hdf5)
            elif suffix in ('.keras', '.h5', '.hdf5') and load_model is not None:
                _image_model = load_model(str(IMAGE_MODEL_PATH), compile=False)
                _image_framework = 'keras'
                print(f"[adapters] ✓ 图片模型已用 Keras 加载 ({IMAGE_MODEL_PATH.name})")
            
            else:
                _image_model = None
                _image_framework = None
                print(f"[adapters] 错误: 不支持的模型格式或缺少加载器: {suffix}")
    ...
    print(f"[adapters] 加载完成 - 问卷: {bool(_survey_artifacts)}, 图片: {bool(_image_model)}, 框架: {_image_framework}, 类别数: {len(_class_names)}")
...
def predict_image(image_bytes: bytes) -> Optional[dict]:
    ...
    try:
        ...
        if _image_framework == 'keras':
            ...
            preds = _image_model.predict(x_in, verbose=0)
        ...
        elif _image_framework == 'torch':
            import torch
            # 转换为 torch tensor (1, C, H, W)
            tensor = _torch.from_numpy(x).permute(2, 0, 1).unsqueeze(0).float()
            
            with _torch.no_grad():
                out = _image_model(tensor)
                ...
                # 应用 softmax
                try:
                    probs = _torch.nn.functional.softmax(out, dim=1).cpu().numpy()[0]
                except Exception:
                    probs = out.cpu().numpy().ravel()
        else:
            return {"error": "未知的图片模型框架"}
